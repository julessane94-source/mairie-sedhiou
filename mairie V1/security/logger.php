<?php
/**
 * Logger avancé — Journalisation multi-niveaux (MySQL)
 */
...
class Logger {
    ...
    private function ensureTable(): void {
        ...
        ");
        try { $this->pdo->exec("CREATE INDEX idx_log_level ON activity_logs(level)");      } catch(PDOException $e) {}
        try { $this->pdo->exec("CREATE INDEX idx_log_user  ON activity_logs(user_id)");    } catch(PDOException $e) {}
        try { $this->pdo->exec("CREATE INDEX idx_log_date  ON activity_logs(created_at)"); } catch(PDOException $e) {}
    }
    ...
    public function log(string $level, string $action, string $details = '', ?int $userId = null): void {
        ...
        $this->writeFile($level, $ts, $userId, $ip, $action, $details);
        ...
        if ($this->pdo && in_array($level, [self::LEVEL_AUDIT, self::LEVEL_SECURITY, self::LEVEL_ERROR], true)) {
            try {
                $this->pdo->prepare("
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ")->execute([$level, $userId, $ip, $ua, $action, $details ?: null, $ts]);
            } catch (Exception $e) {
