    ...
    if ($password !== $confirm_password) {
        ...
        if ($stmt->fetch()) {
            ...
            try {
                ...
                // Créer l'utilisateur (PostgreSQL RETURNING id)
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                $stmt = $pdo->prepare("INSERT INTO users (email, password, nom, prenom, role, telephone) VALUES (?, ?, ?, ?, 'public', ?) RETURNING id");
                $stmt->execute([$email, $hashed_password, $nom, $prenom, $telephone]);
                $user_id = $stmt->fetchColumn();

                // Créer le citoyen avec N°CIT temporaire pour obtenir l'ID de registre
                $stmt = $pdo->prepare("INSERT INTO citoyens (user_id, numero_citoyen, adresse, date_naissance, lieu_naissance) VALUES (?, 'TEMP', ?, ?, ?) RETURNING id");
                $stmt->execute([$user_id, $adresse, $date_naissance, $lieu_naissance]);
                $citoyen_id = $stmt->fetchColumn();

                // Générer le N°CIT définitif : CIT-AAAAMMJJ-NNNNN
                // AAAAMMJJ = date de naissance, NNNNN = numéro de registre (ID citoyen)
                ...
                $pdo->commit();
                $success = "Inscription réussie !<br>Votre <strong>N°CIT</strong> : <code class='fs-5'>" . htmlspecialchars($numero_citoyen) . "</code><br><small class='text-muted'>Conservez ce numéro précieusement.</small>";
...
<div class="container">
    <div class="register-card">
        ...
        <div class="cit-info mb-4">
            ...
            <strong>N°CIT :</strong> Votre numéro de citoyen sera automatiquement généré.
        </div>
        ...
        <form method="POST" id="registerForm">
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    ...
                    <input type="tel" class="form-control" name="telephone" value="<?= htmlspecialchars($_POST['telephone'] ?? '') ?>">
                </div>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="input-group">
                    ...
                    <textarea class="form-control" name="adresse" rows="2"><?= htmlspecialchars($_POST['adresse'] ?? '') ?></textarea>
                </div>
            </div>
